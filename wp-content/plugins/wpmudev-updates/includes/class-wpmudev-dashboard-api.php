...
class WPMUDEV_Dashboard_Api {
	...
	/**
	 * Updates the site_id in the database.
	 *
	 * @since 4.7.4
	 *
	 * @param int $site_id The site_id to store.
	 */
	public function set_site_id( $site_id ) {
		$this->api_site_id = $site_id;
		update_site_option( 'wpmudev_site_id', $site_id );
	}
		...
		if ( 200 == wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_array( $data ) ) {
				if ( isset( $data['membership'] ) && empty( $data['membership'] ) && ! defined( 'WPMUDEV_APIKEY' ) && WPMUDEV_Dashboard::$api->has_key() ) {
					...
					WPMUDEV_Dashboard::$api->set_site_id( null );
				}
