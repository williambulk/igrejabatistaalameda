...
if(!trait_exists('default_methods__Puvox_Software')) 
{
  trait default_methods__Puvox_Software
  {
		public function __construct($arg1=false)
		{
			...
			// if this class is (or not) used as just helper library
			if ($this->isWP)
			{
			...
			if ($this->isWP)
			{
				...
				// initial variables
				$this->my_plugin_vars();
				$this->network_managed	= is_multisite() && $this->getNetworkedState();
				...
				$this->settingsPHP_page	= ( array_key_exists('menu_button_level', $this->static_settings) && $this->static_settings['menu_button_level']=="mainmenu" ) ? 'admin.php' : (is_network_admin() || $this->network_managed ? 'settings.php' : 'options-general.php');
				$this->plugin_page_url_current	= ( is_network_admin() || $this->network_managed ? network_admin_url($this->settingsPHP_page) : admin_url($this->settingsPHP_page) )	.'?page='.$pgnm; 
		...
		*/
		...
		public function fileUrl($file){
			return $this->plugin_URL."/$file?vers_=".$this->filedate($this->plugin_DIR. "/$file");
		}
		...
		// get_remote_data :: removed per WP Reviewer request ( code available at: https://github.com/tazotodua/useful-php-scripts/blob/master/get-remote-url-content-data.php )
		...
		{
			#region - Selector for SETTINGS PAGE
			if(is_network_admin())
			{ 
				...
				<div class="networked_switcher_itsf_parent">
					<div class="networked_switcher_itsf">
						...
						<div class="modeChooser1">
							...
							<input id="subsite" onchange="managed_from_onchanger(this)"  type="radio" name="<?php echo $this->slug;?>[managed_from_site]" value="singlesite" <?php checked(!$this->network_managed);?>  />
							<!--					
		...
		{ 
			...
			{
				$cont .= file_get_contents($this->plugin_DIR.'/'.$each);
			}
		...
		<div class="shortcodes_block">
			...
			<tr>
				...
				<td>
					...
					{ ?>
					<tr>
						<td><code><?php echo $value[0];?></code></td><td><code><?php echo htmlentities($this->truefalse_to_string($value[1]));?></code></td><td><?php echo $value[2];?></td>
					</tr>
					<?php 
					}
					?>
		...
		{ 
			...
			ttLibrary =
			{
				...
				backend_call : function (data, callback)
				{
					...
					data["_wpnonce"] = '<?php echo wp_create_nonce( "ttBackendCall_JS");?>';
		...
		public function ajax_backend_call()
		{
			...
			{
				if( empty( $_POST["_wpnonce"] ) || !wp_verify_nonce( $_POST["_wpnonce"], "ttBackendCall_JS") ) 
				{
		...
		public function admin_scripts_out($hook)  //i.e. edit.php
		{
			...
			$this->register_stylescript($where, 'style',	'wp-jquery-ui-dialog');	//	'ui-css', 'https://code.jquery.com/ui/1.12.1/themes/base/jquery-ui.css',  false,  '1.1');
